/* ===============================
   Unit Definitions (Fully Expanded + Nonlinear)
================================ */
const units = {
...
function convert() {
  ...
  // 비선형 단위 처리
  if (paramUnits[from] && paramUnits[from].toBase && paramUnits[from].fromBase) {
    const base = paramUnits[from].toBase(value);
    const result = paramUnits[to].fromBase(base);
    const formula = paramUnits[to].formula || "";
    // 결과값 녹색, formula 빨간색
    resultText = `<div style="color:green;font-weight:bold">${result.toPrecision(6)} ${to}</div>
                  <div style="color:red;font-size:0.9em">${formula}</div>`;
  } else {
    // 선형 단위 처리
    const base = value * paramUnits[from];
    const result = base / paramUnits[to];
    resultText = `<div style="color:green;font-weight:bold">${result.toPrecision(8)} ${to}</div>`;
  }
